% Works like a charm with collision detection
function PancakeCollectionTest(robot, numPancakes, startPos, EndPos, stopPosition)

    % Initialize x positions for each pancake (starting off-screen to the left)
    xPositions = linspace(startPos, EndPos, numPancakes); % Initial x positions

    y = 1.15; % Fixed y position for all pancakes
    z = 0.67; % Fixed z position to lift pancakes off the floor
    ...
    % Define the collision detection plane (as a Z position or a vertical plane at a specific X position)
    detectionPlaneZ = 0.5; % Example Z position for the plane
        ...
        for i = 1:numPancakes
            ...
            if ~isCarrying && ishandle(h(i))
                if xPositions(i) < stopPosition
                    ...
                    % Check for collision with the detection plane
            ...
            if ~isCarrying && xPositions(i) >= stopPosition && ishandle(h(i))
                ...
                targetPos = [xPositions(i), y, z]; % Adjust z for pickup
            ...
            if isCarrying && ishandle(h(currentPancake))
                ...
                dropOffPos = [stopPosition-0.1, -1, 1.3]; % Define drop-off position
                ...
                finalDropPos = [dropOffPos(1), dropOffPos(2)+1.05, dropOffPos(3)- 0.62 + (pickedCount*0.025)]; % Slightly lower Z to place on surface  
